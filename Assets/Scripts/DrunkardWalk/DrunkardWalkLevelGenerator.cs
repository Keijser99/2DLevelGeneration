...
public class DrunkardWalkLevelGenerator : MonoBehaviour
{
    ...
    [Range(0, 100)]
    public int chestSpawnFrequency;
    [Range(0, 4)]
    public int amountOfWallsSurroundingChest;
    [Range(0, 50)]
    public int maxChestToSpawn;
    ...
    public void SpawnExit()
    {
        ...
        exitSpawnpoint = exitPos;
    }

    private void SpawnChests()
    {
        int randomValue = Random.Range(0, 100);
        int amountOfChestsSpawned = 0;

        for (int x = 0; x < levelWidth - 1; x++)
        {
            for (int y = 0; y < levelHeight - 1; y++)
            {
                int amountOfWallsAroundFloor = 0;
                if (grid[x, y] == LevelTile.floor)
                {
                    if (grid[x, y + 1] == LevelTile.wall)
                    {
                        amountOfWallsAroundFloor++;
                    }
                    ...
                    if (amountOfWallsAroundFloor >= amountOfWallsSurroundingChest)
                    {
                        if (randomValue <= chestSpawnFrequency && amountOfChestsSpawned <= maxChestToSpawn)
                        {
                            if (x != playerSpawnpoint.x && y != playerSpawnpoint.y)
                            {
                                if (x != exitSpawnpoint.x && y != exitSpawnpoint.y)
                                {
                                    Spawn(x, y, chestPrefab);
                                    amountOfChestsSpawned++;
                                }
                            }
                            else
                            {
                                Debug.Log("Oops, tried to spawn on the starting point or on the exit point!");
                            }
                        }
                    }
                }
        ...
        Debug.Log($"Amount of chests spawned: {amountOfChestsSpawned}");
    }
